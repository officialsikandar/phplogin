...
if(isset($_POST['submit'])){
  ...
  if($password === $cpassword){
    ...
    if($row>0){
        ...
        echo '<script>alert("'.$row['username'].' already exist");</script>';
        }
    }
    else{
        ...
        if($result){
          ...
          header("Location: http://localhost/php1/userlogin/index.php");
        }
        else{
        echo "<script>alert('some internal server error');</script>";
        }
    }
  }
  else{
...
if(isset($_POST['login'])){
  ...
  $password = mysqli_real_escape_string($conn, $_POST['password']);
    ...
    if($row == 1){
      ...
      echo "<script>alert('Email or Password does not exist! Please Sign-up First..');</script>";
    }
}
?>
<!DOCTYPE html>
			...
			<div class="nav-item dropdown">
                ...
                <div class="dropdown-menu action-form">
					<form action="index.php" method="post">
						...
						<div class="form-group">
							<input type="cpassword" name="cpassword" class="form-control" placeholder="Confirm Password" required="required">
						</div>
						<div class="form-group">
